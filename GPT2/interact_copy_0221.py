...
def set_interact_args():
    ...
    parser.add_argument('--result_path', default='result.txt', type=str, required=False, help='对话结果路径')
    parser.add_argument('--test_path', default='data/test.txt', type=str, required=False, help='对话结果路径')
...
def main():
    ...
    # test_list =  open("/home/kent/githubfile/GPT2-chitchat/data/testset_eliza.txt").readlines()
    test_list =  open(args.test_path).readlines()
    test_input = test_list[::3]
    gold_reply = test_list[1::3]

    input_list=[]
    chatbot_list=[]
    gold_reply_list=[]

    for index,text in enumerate(test_input):
        if len(text)>70:
            continue
        try:
            # text = input("user:")
            tmp = text.replace("\n","")
            print("user:" + "".join(tmp))
            input_list.append("user:" + "".join(tmp))
            if args.save_samples_path:
            ...
            chatbot_list.append("chatbot:" + "".join(text))
            print("goldreply:" + "".join(gold_reply[index]))
            gold_reply_list.append("gold_reply:" + "".join(gold_reply[index]))
            print()
            if args.save_samples_path:
                samples_file.write("chatbot:{}\n".format("".join(text)))
            file_write_obj = open(args.result_path, 'w')
            for i,j,k in zip(input_list,chatbot_list,gold_reply_list):
