...
def train(model, device, train_list, multi_gpu, args):
    ...
    # scheduler = get_linear_schedule_with_warmup(optimizer, warmup_steps=args.warmup_steps, t_total=total_steps)
    ...
    for epoch in range(args.epochs):
        ...
        if args.train_mmi:  # 当前训练MMI模型
            ...
            model_folder = args.mmi_model_output_path
        else:  # 当前训练对话模型
            model_path = join(args.dialogue_model_output_path, 'model_epoch{}'.format(epoch + 1))
            model_folder = args.dialogue_model_output_path
        if epoch > -1:
            file_list=os.listdir(model_folder)
            if len(file_list)>1:
                shutil.rmtree(join(model_folder,file_list[-1]))
...
def main():
    ...
    if not os.path.exists(args.dialogue_model_output_path):
        os.makedirs(args.dialogue_model_output_path)
    # 创建MMI模型的输出目录
    if not os.path.exists(args.mmi_model_output_path):
        os.makedirs(args.mmi_model_output_path)
    # 加载GPT2模型
