...
def set_interact_args():
    ...
    parser.add_argument('--evalfile', default=None, type=str, required=False, help='选择词库')
    parser.add_argument('--output_file', default=None, type=str, required=False, help='选择词库')
...
def main():
    # userstudy = pd.read_excel("/home/kent/RA/Eliza_userStudy/evaluation_1123.xlsx")
    # user_input = userstudy[userstudy.method=="input"]["content"].to_list()
    # gold_reply = userstudy[userstudy.method=="gold_reply"]["content"].to_list()
    # counselling = userstudy[userstudy.method=="counselling_chatpot_reply"]["content"].to_list()
    # general_chatbot_reply = userstudy[userstudy.method=="general_chatbot_reply"]["content"].to_list()
    # inDomain_chatbot_reply = userstudy[userstudy.method=="inDomain_chatbot_reply"]["content"].to_list()
    args = set_interact_args()
    file_obj  = pd.read_csv(args.evalfile)

    # user_input = [x.strip()[5:] for x in userstudy[::12]] 
    # gold_reply  = [x.strip()[11:] for x in userstudy[9::12]] 
    user_input = file_obj.post.drop_duplicates().to_list()
    
    # gold_reply  = [x for x in userstudy[1::3]] 
    ...
    # loss_df["input"] = user_input
    # loss_df["gold"] = gold_reply
    # loss_df["counselling"] = counselling
    # loss_df["general"]=general_chatbot_reply
    # loss_df["inDomain"] = inDomain_chatbot_reply
    # reply_list=[gold_reply,counselling,general_chatbot_reply,inDomain_chatbot_reply]
    # reply_list=[gold_reply]

    best_candidate=[]
    best_loss=[]
    for index_lbk, user_post in enumerate(user_input):
    # for i,j in zip(user_input,reply):
        print("user:"+user_post)
        tmpReply_all = file_obj[file_obj.post==user_post].candidate.drop_duplicates().to_list()
        reply_gap=len(tmpReply_all)//10
        tmp_candidate=[]
        tmp_loss=[]
        for gap_index in range(reply_gap):
            tmpReply=tmpReply_all[gap_index*10:(gap_index+1)*10]
            if gap_index == reply_gap-1:
                tmpReply=tmpReply_all[gap_index*10:]
            ...
            try:
                ...
                input_ids = [copy.deepcopy(input_ids) for _ in range(len(tmpReply))]
                ...
                for _ in range(args.max_len):
                    ...
                    for index in range(len(tmpReply)):
                    ...
                    for j_index, j in enumerate(tmpReply):
                        a=tokenizer.tokenize(j)
                        b = tokenizer.convert_tokens_to_ids(a)
                        if _ < len(b):
                            # print(next_token)
                            # print(b[_])
                            next_token[j_index][0]=b[_]
                            # print(_)

                        else:
                            next_token[j_index][0]=102
                        # print(102)
                ...
                for batch_index in range(len(tmpReply)):
                    response = []
                ...
                # candidate_responses=candidate_responses[1:]
                # candidate_responses.append("我顶你个肺")
                ...
                min_loss_list =[]
                for index,response in enumerate(candidate_responses):
                ...
                logger.info("chatbot:" + "".join(text))
                tmp_candidate.append("".join(text))
                tmp_loss.append(min_loss)
                # if args.save_samples_path:
                ...
                #     samples_file.close()
                break
        best_loss.append(np.min(tmp_loss))
        best_candidate.append(tmp_candidate[np.argmin(tmp_loss)])
    output_data=pd.DataFrame()
    output_data["post"] = user_input
    output_data["dialogpt"] = best_candidate
    output_data["loss"] =best_loss
    output_data.to_csv(args.output_file,encoding="utf_8_sig", index=False)
    #     loss_df["loss"+str(index_lbk)]=tmp
    # loss_df.to_csv(args.output_file,encoding="utf_8_sig", index=False)
