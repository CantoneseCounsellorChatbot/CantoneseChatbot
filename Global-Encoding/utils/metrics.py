...
def rouge_lbk(reference, candidate, log_path, print_log, config):
    assert len(reference) == len(candidate)
    ...
    for i in range(len(reference)):
        with codecs.open(ref_dir+"%06d_reference.txt" % i, 'w', 'utf-8') as f:
            f.write(" ".join(list(reference[i][0])).replace(' <\s> ', '\n') + '\n')
        with codecs.open(cand_dir+"%06d_candidate.txt" % i, 'w', 'utf-8') as f:
            f.write(" ".join(candidate[i]).replace(' <\s> ', '\n').replace('<unk>', 'UNK') + '\n')
    ...
    candidate_list=[]
    for i in range(len(reference)):
        reference_list.append(" ".join(list(reference[i][0])).replace(' <\s> ', '\n') + '\n')
        candidate_list.append(" ".join(candidate[i]).replace(' <\s> ', '\n').replace('<unk>', 'UNK') + '\n')

    rouge = Rouge()
    rouge_score = rouge.get_scores(candidate_list, reference_list)
    tmp = pd.DataFrame(rouge_score)
    rouge1=pd.DataFrame(tmp["rouge-1"].tolist())
    rouge2=pd.DataFrame(tmp["rouge-2"].tolist())
    rougeL=pd.DataFrame(tmp["rouge-l"].tolist())
    recall=[round(rouge1.r.mean() * 100,2 ),
            round(rouge2.r.mean() * 100,2 ),
            round(rougeL.r.mean() * 100,2 )]
    precision=[round(rouge1.p.mean() * 100,2 ),
            round(rouge2.p.mean() * 100,2 ),
            round(rougeL.p.mean() * 100,2 )]
    f_score=[round(rouge1.f.mean() * 100,2 ),
            round(rouge2.f.mean() * 100,2 ),
            round(rougeL.f.mean() * 100,2 )]
    print_log("F_measure: %s Recall: %s Precision: %s\n"
              % (str(f_score), str(recall), str(precision)))
