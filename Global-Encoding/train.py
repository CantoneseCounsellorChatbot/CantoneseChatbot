...
def eval_model(model, data, params):
    ...
    if opt.mode =="eval":
        if not os.path.exists(opt.resultF):
            os.makedirs(opt.resultF)
        with codecs.open(opt.resultF+'/candidate.txt','w+','utf-8') as f:
            for i in range(len(candidate)):
                f.write(" ".join(candidate[i])+'\n')
...
def build_log():
    # log
    if not os.path.exists(opt.logF):
        os.mkdir(opt.logF)
    if opt.log == '':
        log_path = opt.logF + str(int(time.time() * 1000)) + '/'
    else:
        log_path = opt.logF + opt.log + '/'
